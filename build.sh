...
then
	...
	then
		...
		make -C ${BUILDROOT} defconfig BR2_DEFCONFIG=${MODIFIED_DEFCONFIG_REL_BUILDROOT} BR2_EXTERNAL=${EXTERNAL_REL_BUILDROOT}
	else
		echo "Using default defconfig ${DEFAULT_DEFCONFIG}"
		make -C ${BUILDROOT} defconfig BR2_DEFCONFIG=${DEFAULT_DEFCONFIG} BR2_EXTERNAL=${EXTERNAL_REL_BUILDROOT}
